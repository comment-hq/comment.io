...
| Supplied comm URL plus authenticated HTTPS | If a clean shortlink hides slug/token, resolve it without Authorization or redirects, follow its confirmation once, and accept the token-bearing `/d/{slug}` Location only when final origin and slug match its `api_reference_url` and `slug`. At that final origin, send `X-Comment-Share-Ingress: 1` on the first per-comm `?docs` GET, switch to returned `your_token`, and read immediately |
| Comm URL or slug, but only URL fetch (no browser UI/headers) | Use any supplied share URL now; add `mode=agent` without removing its query and follow one confirmation. For a bare slug/token-free URL, first try `/d/{slug}?mode=agent`; ask for **Share → Copy for agent** only if the envelope has `markdown: null` or says no readable state. The envelope is read-only |
